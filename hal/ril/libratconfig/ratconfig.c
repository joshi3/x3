...
static int checkRatConfig(unsigned int iRat) {
    ...
    if ((iRat | maxrat) == maxrat) {
        ...
        RLOGD("checkRatConfig: not supported (%X/%X)", iRat, maxrat);
        return FALSE;
    }
}
...
static unsigned int getRatConfig() {
    ...
    if (strlen(rat) > 0) {
        ...
        if (checkRatConfig(actived_rat) == FALSE) {
            RLOGD("getRatConfig: illegal value of PROPERTY_RAT_CONFIG, set max");
            actived_rat = getMaxRat();
        }
    } else {
        RLOGD("getRatConfig: active rat is not exist, initialize");
        actived_rat = getMaxRat();
    }
    return actived_rat;
}
