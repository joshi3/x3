...
typedef struct sph_param_buf_t {
    uint16_t memory_size;       /* buffer size (memory) */
    uint16_t data_size;         /* 0 <= data_size <= memory_size */
...
enum {
    ...
    SPH_BT_CVSD_MSBC    /* via software BT codec */
};
...
enum {
    ...
    SPH_PARAM_VIA_SHM       /* speech param is written in share memory */
};
...
typedef struct sph_info_t {
    ...
    uint8_t  sph_param_valid;    /* SPH_PARAM_VIA_SHM only. 0: invalid, 1: valid */
    ...
    uint8_t  __reserve_at_32byte[12];
    uint8_t  __reserve_at_48byte[16];
    uint8_t  __reserve_at_64byte[16];
} sph_info_t; /* 64 bytes */

#define SIZE_OF_SPH_INFO (64)
...
#define SPEECH_SHM_SIZE (52 * 1024)
#define SPEECH_SHM_GUARD_REGION_SIZE (32)
#define SPEECH_SHM_SPEECH_PARAM_SIZE (12 * 1024)
#define SPEECH_SHM_AP_DATA_SIZE (8  * 1024)
#define SPEECH_SHM_MD_DATA_SIZE ((32 * 1024) - 160) /* 160: SHM total 52K only... */


typedef struct sph_shm_t {
    /* 32 bytes gurard region */
    uint8_t guard_region_pre[SPEECH_SHM_GUARD_REGION_SIZE];
    ...
    uint8_t sph_param[SPEECH_SHM_SPEECH_PARAM_SIZE];

    /* 8K bytes AP data */
    uint8_t ap_data[SPEECH_SHM_AP_DATA_SIZE];

    /* 32K bytes MD data */
    uint8_t md_data[SPEECH_SHM_MD_DATA_SIZE];

    /* 32 bytes gurard region */
    uint8_t guard_region_post[SPEECH_SHM_GUARD_REGION_SIZE];
} sph_shm_t;
