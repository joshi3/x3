...
int getSimCount() {
    ...
    property_get("persist.radio.multisim.config", tempstr, "dsds");
    if (strcmp(tempstr, "ss") == 0) {
        simCount = 1;
    } else if (strcmp(tempstr, "dsds") == 0 ||
            strcmp(tempstr, "dsda") == 0) {
        simCount = 2;
...
int isSupportCommonSlot() {
    ...
    char property_value[PROPERTY_VALUE_MAX] = { 0 };
    property_get("ro.mtk_sim_hot_swap_common_slot", property_value, "0");
    LOGD("ro.mtk_sim_hot_swap_common_slot:[%d]", atoi(property_value));
...
error:
    ...
    LOGD("[maskString] error[%d]", stgLen);
    return;
}
...
unsigned char* pii(char* input) {
    ...
    if (!EVP_EncodedLength(&outLength, sizeof(sha1Result))) {
        LOGD("pii: can not get length of base64");
        return NULL;
    }
    outText = (unsigned char *) OPENSSL_malloc(outLength);
    if (outText == NULL) {
        LOGD("pii: malloc fail");
        return NULL;
    }
...
int readMccMnc(RIL_SOCKET_ID rid, char *mccmnc) {
    ...
    property_get(prop, mccmnc, "0");
    if (strcmp("N/A", mccmnc) == 0) {
        ret = READ_MCC_MNC_ERR_NOT_READY;
    }
    LOGD("prop %s, readMccMnc %s, ret %d", prop, mccmnc, ret);
    free(prop);
    return ret;
}
...
int isDisableCapabilitySwitch() {
    ...
    if (ret == 0) {
        property_get("persist.radio.simswitch.emmode", property_value, "1");
        ret = atoi(property_value);
        if (ret == 0) {
            ret = 1;
        } else {
...
GEMINI_MODE getGeminiMode() {
    ...
    if (sProjType == GEMINI_MODE_UNKNOWN) {
        ...
        if (isLteSupport()) {
            ...
            if (strchr(propValue, 'L') != NULL) {
                LOGD("getProjectType, project config error.");
            } else if (strchr(propValue, 'W') != NULL) {
    ...
    LOGD("getProjectType = %d", sProjType);
    return sProjType;
}
...
const char* mtkRequestToString(int request) {
    switch(request) {
        ...
        /// M: CC: For 3G VT only
        case RIL_UNSOL_VT_STATUS_INFO: return "RIL_UNSOL_VT_STATUS_INFO";
        ...
        case RIL_REQUEST_CURRENT_STATUS: return "RIL_REQUEST_CURRENT_STATUS";
        /// @}
...
int isEpdgSupport() {
    char property_value[PROPERTY_VALUE_MAX] = { 0 };
    property_get("persist.mtk_epdg_support", property_value, "0");
    return atoi(property_value);
}

int getMultiImsSupportValue() {
    char multiImsSupport[PROPERTY_VALUE_MAX] = { 0 };
    property_get("persist.mtk_mims_support", multiImsSupport, "1");
...
int getRcsSrvTag() {
    ...
    char property_value[PROPERTY_VALUE_MAX] = { 0 };
    property_get("persist.service.tag.rcs", property_value, "0");
    rcsSrvTags = atoi(property_value);
    memset(property_value, 0, sizeof(property_value));
    property_get("persist.customized.rcs", property_value, "0");
    rcsCustomized = atoi(property_value);
    RLOGI("rcsCustomized: %d, rcsSrvTags: %d", rcsCustomized, rcsSrvTags);
...
int isOp12Support() {
    ...
    property_get("persist.operator.optr", optr_value, "0");
...
int isOp12MccMnc(char *mccmnc) {
    ...
    for (i = 0; i < sizeof(VZW_MCC_MNC) / sizeof(VZW_MCC_MNC[0]); i++) {
        if (0 == strcmp(mccmnc, VZW_MCC_MNC[i])) {
            RLOGD("isOp12MccMnc: true.");
            return 1;
        }
    }
    RLOGD("isOp12MccMnc: false");
    return 0;
};

int isOp16Support() {
    int isSupport = 0;
    char dsbp_enable[PROPERTY_VALUE_MAX] = {0};
    char optr[PROPERTY_VALUE_MAX] = {0};

    property_get("persist.operator.optr", optr, "");

    // Check D-SBP enabled or not
    property_get("persist.radio.mtk_dsbp_support", dsbp_enable, "");
    // Support D-SBP
    if (!strcmp(dsbp_enable, "1")) {
        // get dynamic SBP ID by SIM
        char dsbp[PROPERTY_VALUE_MAX] = {0};
        property_get("persist.radio.sim.sbp", dsbp, "0");
...
int isOp01Volte() {
    char sbp_value[PROPERTY_VALUE_MAX] = {0};
    int isOp01Volte = 0;

    property_get("ro.mtk_md_sbp_custom_value", sbp_value, "");
    if (strlen(sbp_value) > 0) {
        ...
        if (tmpValue == 1) {    // CMCC customize load
            ...
            if (!strcmp(dsbp_prop, "1")) { // support D-SBP
                // get dynamic SBP ID by SIM
                property_get("persist.radio.sim.sbp", sbp_value, "0");
...
int isApFallbackSupport() {
    ...
    property_get("persist.ap.fallback.support", value, "100");
    ret = atoi(value);
...
int isMdFallbackSupport() {
    ...
    int ret = 0;
    int default_value = 100; // Default value.
    property_get("persist.md.fallback.support", value, "100");
    ret = atoi(value);
    if (default_value == ret) {
        if (isOp07Support()) {
            ret = 1;
...
bool isFallbackNotSupportByOp() {
    ...
    property_get("gsm.operator.numeric", operatorNumeric, "");

    for (i = 0; i < length; i++) {
        if (0 == strcmp(operatorNumeric, VZW_MCC_MNC[i])) {
            LOGD("isFallbackNotSupportByOp: true");
            ret = true;
            return ret;
        }
    }
...
static const char PROPERTY_SIM_SWITCH_FULL_UICC_TYPE[2][25] = {
...
static bool isCTCardType(int slot_id) {
    ...
    int i;
    property_get(PROPERTY_SIM_SWITCH_FULL_UICC_TYPE[slot_id], tmp, "");
    ...
    if (value1 == NULL && value2 == NULL) {
        if (value3 != NULL) {
            property_get(PROPERTY_SIM_SWITCH_CT3G[slot_id], tmp2, "");
...
bool hasVsimCard() {
    for (int rid = RIL_SOCKET_1; rid < RIL_SOCKET_NUM; rid++) {
        char persist_vsim_prop[PROPERTY_VALUE_MAX] = {0};
        char vsim_enabled_prop[PROPERTY_VALUE_MAX] = {0};
        char vsim_inserted_prop[PROPERTY_VALUE_MAX] = {0};
        getMSimProperty(rid, "persist.radio.external.sim", persist_vsim_prop);
...
void setRildInterfaceCtrlSupport() {
    if (isEpdgSupport()) {
        RLOGI("set ril.epdg.interface.ctrl 1");
        property_set("ril.epdg.interface.ctrl", "1");
    }
}

int getRildInterfaceCtrlSupport() {
    char value[PROPERTY_VALUE_MAX] = {0};
    property_get("ril.epdg.interface.ctrl", value, "0");
...
int isMultipleImsSupport() {
    ...
    if (ims_num == -1) {
        char property_value[PROPERTY_VALUE_MAX] = { 0 };
        property_get("persist.mtk_mims_support", property_value, "0");
...
int getClassType(char *apn, int supportedApnTypesBitmask) {
    ...
    if ((supportedApnTypesBitmask & RIL_APN_TYPE_EMERGENCY) || strcasecmp(VZW_EMERGENCY_NI, apn) == 0) {
        ...
        LOGD("getClassType: set to default class 3");
    }

    LOGD("[%s] classType = %d", __FUNCTION__, classType);
    return classType;
}
...
int isExternalSimSupport() {
    ...
    RLOGI("[isExternalSimSupport] vsim support:%d", atoi(property_value));
    return atoi(property_value);
}
...
int isPersistExternalSimDisabled() {
    char property_value[PROPERTY_VALUE_MAX] = { 0 };
    property_get("ro.mtk_persist_vsim_disabled", property_value, "0");
    return atoi(property_value);
}
// External SIM [End]
...
bool isDssNoResetSupport() {
    ...
    property_get("ril.simswitch.no_reset_support", tmp, "0");
    if (strcmp(tmp, "1") == 0) {
        return true;
    }
    return false;
...
int needToHidenLog(const char *target) {
    for (int i = 0; i < NUM_ELEMS(sAtCmdWithoutContent); i++) {
