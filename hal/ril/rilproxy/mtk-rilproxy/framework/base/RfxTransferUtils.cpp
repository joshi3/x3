...
DataToParcelInfo* RfxTransferUtils::findParcelInfoFromRequestTable(int request) {
    ...
    RFX_LOG_E(LOG_TAG, "[RFX] Can not find request in data_to_parcel table");
    return NULL;
}
...
Parcel* carrierRestrictionsToParcel(int request, int token, void* data,
    ...
    RFX_UNUSED(datalen);
    fillHeader(parcel, request, token);
    RIL_CarrierRestrictions *args = (RIL_CarrierRestrictions *) data;
...
static void parcelToDataCallList(RIL_Token t, RIL_Errno e, int id, Parcel* p, int slotId) {
    ...
    if (isResponseType(type)) {
        ...
        // URC
        RESPONSE_TO_RILJ(id, list, sizeof(RIL_Data_Call_Response_v11) * num,
...
static void parcelToCarrierRestrictions(RIL_Token t, RIL_Errno e, int id, Parcel* p, int slotId) {
    ...
    if (isResponseType(type)) {
        // Response
        if (e == 0) {
            RfxRilAdapter::responseToRilj(t, e, &entry, sizeof(RIL_CarrierRestrictions));
        } else {
            RfxRilAdapter::responseToRilj(t, e, NULL, 0);
        }
    } else {
        // URC
        RESPONSE_TO_RILJ(id, &entry, sizeof(RIL_CarrierRestrictions), RIL_SOCKET_ID(slotId));
    }
}
    ...
    switch(rilCellIdentity.cellInfoType) {
        ...
        case RIL_CELL_INFO_TYPE_LTE: {
            ...
            rilCellIdentity.cellIdentityLte.pci =
                    (int) convertResponseHexStringEntryToInt(response, 7, numStrings);
