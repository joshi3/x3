...
void RpAudioControlHandler::setRefreshModem(bool isSvlte,
    ...
    int isModeReady = 0;
    String8 phoneCallStatusReadyStr = String8("getPhoneCallStatus=1");

    /**
     * get audio mode (always AUDIO_MODE_IN_CALL==2)
     * get phone call status (ready==1, not ready==0)
     */
    String8 currentAudioModeStr =
            android::MTKPrimaryDevicesHalClient::getInstance()->getParameters(
                    String8("getMode="));
    String8 currentPhoneCallStatusStr =
            android::MTKPrimaryDevicesHalClient::getInstance()->getParameters(
                    String8("getPhoneCallStatus="));

    if ((currentAudioModeStr == String8("getMode=2"))
            && (currentPhoneCallStatusStr == phoneCallStatusReadyStr)) { // status ready
    ...
    if (!isModeReady) {
        RFX_LOG_E(RP_AUDIOCONTROL_TAG,
                "%s[%d] mode not ready, AudioMode(%s), PhoneCallStatus(%s)",
                (isSvlte ? "svlte setRefreshModem" : "setRefreshModem"),
                parameter->slotId,
                currentAudioModeStr.string(),
                currentPhoneCallStatusStr.string());
        return;
    }
