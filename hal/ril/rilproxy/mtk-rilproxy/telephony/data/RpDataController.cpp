...
#define PROPERTY_DEFAULT_DATA_ICCID "persist.radio.data.iccid"
...
RpDataController::RpDataController() :
    ...
    mImsPdnFound(false), mIsCheckPdnReuse(false),
...
void RpDataController::onInit() {
    ...
    if (NULL == mCcmniData[m_slot_id]) {
        ...
        if (result < 0) {
            ...
            logD(RFX_LOG_TAG, "main: process id %d, thread id = %d", getpid(), pthread_self());
            logD(RFX_LOG_TAG, "mNetdLooperTid = %d", tidNetdLooper);
        }
