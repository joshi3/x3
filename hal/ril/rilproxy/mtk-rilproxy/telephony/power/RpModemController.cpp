...
const char PROPERTY_BUILD_RAT_CONFIG[40] = "ro.mtk_protocol1_rat_config";
...
void RpModemController::calculateRemoteSim(int cardType1, int cardType2, int cdmaSlot) {
    ...
    RLOGD("calculateRemoteSim, cardType1:%d, cardType2:%d, persist.radio.simswitch:%s, cdmaSlot:%d",
            cardType1, cardType2, temp, cdmaSlot);

    mModemStatus = 2;
    mRemoteProtocol = 1;
    while (1) {
        if (!isOP09ASupport() && RpFeatureOptionUtils::getSimCount() >= 2) {
            ...
            RLOGD("configModemStatus: Solution1 for OP09 or SS[%d]", RpFeatureOptionUtils::getSimCount());
    ...
    if (isSvlteSupport()) {
        ...
        if (emdstatusMessage->gsmRequest != NULL && emdstatusMessage->cdmaRequest != NULL) {
            ...
            if (ret) {
                ...
                RLOGD("SVLTE ril.cdma.emdstatus.send is set to 1");
        ...
        property_set(emdstatusProperty, "1");
        RLOGD("ril.cdma.emdstatus.send is set to 1");
