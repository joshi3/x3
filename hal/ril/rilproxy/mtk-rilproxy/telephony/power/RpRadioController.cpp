...
void RpRadioController::suggestedCapability(int desiredCapability, RpSuggestRadioCapabilityCallback &callback) {
    ...
    if (((desiredCapability == RIL_CAPABILITY_NONE) || (desiredCapability != RIL_CAPABILITY_NONE && requestPower )) &&
            (!inDynamicSwitchRadioOff) && ((oldCapability != desiredCapability) || isCTLteTddTestSupport(slotId))) {
...
bool RpRadioController::isCTLteTddTestSupport (int slotId) {
    int card_types = 0;
    card_types = getStatusManager()->getIntValue(RFX_STATUS_KEY_CARD_TYPE, CARD_TYPE_NONE);
    RLOGD("card_types %d. ", card_types);
