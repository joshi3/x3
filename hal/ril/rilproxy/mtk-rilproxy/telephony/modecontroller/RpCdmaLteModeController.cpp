...
int RpCdmaLteModeController::getCurrentNetworkType(int slotId) {
    ...
    property_get("ro.boot.opt_lte_support", tempstr, "0");
    ...
    if (mSwitchInfo->card_modes[slotId] == RADIO_TECH_MODE_CDMALTE) {
        ...
        if (mSwitchInfo->isCt3GDualMode[slotId]) {
            ...
            if (slotId == capabilitySlotId) {
                ...
                RFX_LOG_D(RP_CDMALTE_MODE_TAG, "[SMC][getCurrentNetworkType] C card slotCapability: %d,"
                        " netWorkType: %d", slotCapability, netWorkType);
            }
        } else {
            // G Card Case:
            if (slotId == capabilitySlotId) {
    ...
    bool isCTLteTddTestCase = isCTLteTddTestSupport(slotId);
    if (isCTLteTddTestCase) {
        netWorkType = PREF_NET_TYPE_LTE_CDMA_EVDO;
    }
    RFX_LOG_D(RP_CDMALTE_MODE_TAG, "[SMC][getCurrentNetworkType]isLteSupport: %d,"
            " capabilitySlotId: %d, isCTLteTddTestCase: %d, netWorkType: %d, getIs4GCCard[%d]: %d",
            isLteSupport, capabilitySlotId, isCTLteTddTestCase, netWorkType, slotId,
...
bool RpCdmaLteModeController::isCTLteTddTestSupport(int slotId) {
    if (sIsSvlteSupport && slotId == 0 && mSwitchInfo->card_types[slotId] == CARD_TYPE_USIM) {
        char tempstr[PROPERTY_VALUE_MAX] = { 0 };
