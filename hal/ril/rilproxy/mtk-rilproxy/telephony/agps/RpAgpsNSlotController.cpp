...
bool RpAgpsNSlotController::isCtSimCard() {
    bool ret = false;
    int cdma_card_type = getStatusManager(
            getDefaultDataSlotId())->getIntValue(RFX_STATUS_KEY_CDMA_CARD_TYPE);
    if (cdma_card_type == CT_4G_UICC_CARD ||
        cdma_card_type == CT_UIM_SIM_CARD ||
        cdma_card_type == CT_3G_UIM_CARD) {
        ret = true;
    }
    AGPS_LOGD("RpAgpsNSlotController::isCtSimCard(%d)", cdma_card_type);
    return ret;
}
