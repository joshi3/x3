...
bool RpIrUtils::isWorldModeSupport() {
    ...
    property_get("ro.mtk_md_world_mode_support", strProp, "0");
...
bool RpIrUtils::isViceSimSupportLte() {
    char strProp[PROPERTY_VALUE_MAX] = {0};
    property_get("persist.radio.mtk_ps2_rat", strProp, "");
...
int RpIrUtils::getDefaultDataSim() {
    // return value is 0,1,2...
    int i = 0;
    int strLen = 0;
    char iccid[PROPERTY_VALUE_MAX] = {0};
    char defaultDataIccid[PROPERTY_VALUE_MAX] = {0};
    char maskIccid[PROPERTY_VALUE_MAX] = {0};
    int simCount = sizeof(PROPERTY_ICCID_SIM)/sizeof(PROPERTY_ICCID_SIM[0]);

    property_get("persist.radio.data.iccid", defaultDataIccid, "");
    if (strlen(defaultDataIccid) == 0 || strcmp("N/A", defaultDataIccid) == 0) {
        RFX_LOG_D(RFX_LOG_TAG , "Default Data not set");
        return 0;
    }
    strLen = strlen(defaultDataIccid);
    strncpy(maskIccid, defaultDataIccid, strLen);
    memset(maskIccid, '*', 12 * sizeof(char));
    RFX_LOG_D(RFX_LOG_TAG , "Default data ICCID = %s", maskIccid);

    for (i = 0; i < simCount; i++) {
        property_get(PROPERTY_ICCID_SIM[i], iccid, "");
        if (strlen(iccid) == 0 || strcmp("N/A", iccid) == 0) {
            RFX_LOG_D(RFX_LOG_TAG , "SIM%d not inserted", i);
            continue;
        }
        if (strStartsWith(iccid, defaultDataIccid)) {
            strLen = strlen(iccid);
            strncpy(maskIccid, iccid, strLen);
            memset(maskIccid, '*', 12 * sizeof(char));
            RFX_LOG_D(RFX_LOG_TAG , "Default data is SIM%d (from 0), ICCID = %s", i, maskIccid);
            return i;
        }
    }
    RFX_LOG_D(RFX_LOG_TAG , "Not found");
