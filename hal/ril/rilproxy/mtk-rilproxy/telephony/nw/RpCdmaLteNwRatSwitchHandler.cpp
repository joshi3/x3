...
void RpCdmaLteNwRatSwitchHandler::onInit() {
    ...
    property_get("ril.active.md", active_modem, "0");
...
void RpCdmaLteNwRatSwitchHandler::requestSetEvdoMode(const int prefNwType ) {
    logD(CDMA_LTE_RAT_CTRL_TAG, "Step2: Config evdo mode if needed");
    ...
    if (evdoMode != EVDO_MODE_INVALID) {
        ...
        logD(CDMA_LTE_RAT_CTRL_TAG, "requestSetEvdoMode(), set evdo mode to %d.", evdoMode);
        mNwRatController->requestToRild(message);
    } else {
        // no need to config EHRPD
        logD(CDMA_LTE_RAT_CTRL_TAG, "no need to set evdo mode, prepare to set rat mode ");
        requestSetRatMode();
    }
}
...
void RpCdmaLteNwRatSwitchHandler::requestSetRatMode() {
    logD(CDMA_LTE_RAT_CTRL_TAG, "Step3: Set Rat mode");
    ...
    logD(CDMA_LTE_RAT_CTRL_TAG,
            "requestSetRatMode(), radioMode is %d, preferNwType is %d, newRatMode is %d, preNwsMode is %d, newNwsMode is %d, is3gCdmaSim is %s",
    ...
    if (radioMode != RADIO_TECHNOLOGY_MODE_UNKNOWN
        ...
        logD(CDMA_LTE_RAT_CTRL_TAG, "requestSetRatMode , invalid prefNwType, update switch done");
...
void RpCdmaLteNwRatSwitchHandler::responseSetRatMode(
    ...
    if (error != RIL_E_SUCCESS) {
        ...
        // 5, set radio power
        logD(CDMA_LTE_RAT_CTRL_TAG, "Step5: Power on Radio according to the prefer nw type");
...
void RpCdmaLteNwRatSwitchHandler::onNwRatSwitchDone(
    ...
    // M: if execute ERAT fail in call, set the key value to fail, other case, set to success.
    if (errorCode == RIL_E_OPERATION_NOT_ALLOWED) {
        mNwRatController->getStatusManager()->setIntValue(RFX_STATUS_KEY_RAT_SWITCH_STATUS, 0);
    } else {
        mNwRatController->getStatusManager()->setIntValue(RFX_STATUS_KEY_RAT_SWITCH_STATUS, 1);
    }
...
bool RpCdmaLteNwRatSwitchHandler::needCdmaRadioOn(const int prefNwType, const NwsMode nwsMode) {
    ...
    if (isCTLteTddTestSupport(mSlotId)) {
        return false;
    }
    if (mVolteState == VOLTE_ON) {
        return false;
...
RatSwitchCaller RpCdmaLteNwRatSwitchHandler::getRatSwitchCaller(){
    logD(CDMA_LTE_RAT_CTRL_TAG, "getRatSwitchCaller(), mRatSwitchCaller is %d. ",
            mRatSwitchCaller);
    return mRatSwitchCaller;
}

bool RpCdmaLteNwRatSwitchHandler ::isCTLteTddTestSupport (int slotId) {
    int card_types = mNwRatController->getStatusManager()->getIntValue(RFX_STATUS_KEY_CARD_TYPE,
            CARD_TYPE_NONE);
    if (slotId == 0 && card_types == CARD_TYPE_USIM) {
        char tempstr[PROPERTY_VALUE_MAX] = { 0 };
        property_get("persist.sys.forcttddtest", tempstr, "0");
        if (tempstr[0] == '1') {
            return true;
        }
    }
