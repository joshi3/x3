...
void RpGsmNwRatSwitchHandler :: onInit () {
    ...
    property_get("ro.boot.opt_c2k_lte_mode", prop_svlte_support, "0");
...
void RpGsmNwRatSwitchHandler::responseSetPreferredNetworkType(const sp<RfxMessage>& response) {
    ...
    if (error != RIL_E_SUCCESS) {
        // error
        logD(GSM_RAT_CTRL_TAG, "responseSetPreferredNetworkType(), error is %d. sSwitch:%d",
                error, RpNwRatController::getSwitchState());
        mNwRatController->responseToRilj(response);
    } else {
        // ok
        if (response->getSource() == RADIO_TECH_GROUP_GSM) {
            logD(GSM_RAT_CTRL_TAG, "responseSetPreferredNetworkType(), response OK, sSwitch:%d",
...
void RpGsmNwRatSwitchHandler::onNwRatSwitchDone(
    ...
    if (mNwRatController->isEratExtSupport()) {
        if (mMdSettings.prefNwType != -1) {
            ...
            mNwRatController->updateState(mMdSettings.nwsMode, mMdSettings.prefNwType, mMdSettings.volteState);
            ...
            if (mAction != NULL) {
                logD(GSM_RAT_CTRL_TAG, "onNwRatSwitchDone, do the action...");
                mAction->act();
            } else {
                logD(GSM_RAT_CTRL_TAG, "onNwRatSwitchDone, mAction is null");
            }
