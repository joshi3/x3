...
bool RpCdmaSmsCtrl::handleImsSendCdmaSms(const sp <RfxMessage>& message) {
    ...
    uint8_t retry;
    int32_t messageRef;
    RIL_IMS_SMS_Message *imsSendSms;
    RIL_CDMA_SMS_Message cdmaSmsMsg;
...
bool RpCdmaSmsCtrl::handleCdmaSmsSpecificToGsmRsp(const sp<RfxMessage>& message) {
    ...
    if (mIsHandlingImsMoSms && localReqRsp.getType() == CDMA_SMS_LOCAL_REQ_CMGS_RSP) {
        ...
        if (localReqRsp.getResult()) {
            ...
            // failed
            if (localReqRsp.getErrorClass() == 2 ||
                    localReqRsp.getErrorClass() == 3) {
                // 2: IMS temporary failure
                // 3: IMS permanent failure
                // While sending out the SEND_FAIL_RETRY to RILJ, SMS frameworks may do re-try on
                // AOSP default flow
                imsSmsRsp = RfxMessage::obtainResponse(RIL_E_SMS_SEND_FAIL_RETRY, message);
            } else {
                imsSmsRsp = RfxMessage::obtainResponse(RIL_E_GENERIC_FAILURE, message);
            }
        }
...
void RpCdmaSmsCtrl::genericErrorRspToRilj(const sp <RfxMessage>& message) {
    sp<RfxMessage> rsp = RfxMessage::obtainResponse(RIL_E_GENERIC_FAILURE, message);
    responseToRilj(rsp);
}
